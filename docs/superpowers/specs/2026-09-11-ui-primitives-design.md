...
| Three bundled dependencies on a PWA | the app is installable and must stay light | **Measured (Task 7): precache is 809.54 KiB (16 entries) against the 630.41 KiB budget — +179.13 KiB, +28.4%, over the ~15% threshold. This is a finding, not filed quietly.** Rebuilding the actual pre-Task-1 commit (`6898313`) from its own lockfile reproduces 364.48 KiB, not 630.41 KiB — the documented budget does not match what this branch's own history produces, and that discrepancy is unresolved. Of the growth, ~64 KiB (`@radix-ui/react-popover`) reaches every customer by design, since `DateField`/`DateJump` are used on `/prenota`; ~45 KiB (`dnd-kit`) and ~24 KiB (`@radix-ui/react-select`) stay inside admin-only lazy chunks. An experiment forcing the vendor libraries into explicit `manualChunks` (not applied, scope of this task was measurement only) recovered ~282 KiB of code the default build duplicates across chunks, bringing the total to ~527.51 KiB — recoverable, and worth a follow-up. Detail in `.superpowers/sdd/2026-09-11-ui-primitives/task-7-report.md`. |
