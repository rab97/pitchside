...
-- PS015 il member non appartiene alla struttura del campo
...
begin
  ...
  -- Il member deve appartenere alla stessa struttura del campo: vale per
  -- tutti, gestore compreso. Senza questo controllo un cliente della
  -- struttura A passa il proprio member_id e prenota su B, e la riga in
  -- bookings nasce con facility_id di B e un member di A.
  if not exists (
    select 1 from public.members m
    where m.id = p_member_id and m.facility_id = fac.id
  ) then
    raise exception 'Il cliente non appartiene a questa struttura.'
      using errcode = 'PS015';
  end if;
